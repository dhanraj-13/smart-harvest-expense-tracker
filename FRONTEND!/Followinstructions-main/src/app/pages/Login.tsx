...
export function Login() {
  ...
  const passwordError =
    passwordTouched && password.trim().length > 0 && !PASSWORD_REGEX.test(password)
      ? t("login.errorPasswordRules")
      : "";
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (!PASSWORD_REGEX.test(password)) {
      ...
      setError(t("login.errorPasswordRules"));
      return;
    }
    setError("");
    try {
          ...
          <div>
            ...
            <div className="relative">
              <input
                ...
                className="w-full rounded-lg px-3.5 py-2.5 pr-11 outline-none"
                style={{
                  border: `1.5px solid ${SH.border}`,
                  backgroundColor: "rgba(3, 17, 13, 0.62)",
                  color: SH.text,
              ...
              <button
                ...
                style={{ backgroundColor: "rgba(255,255,255,0.92)", color: "#333" }}
              >
                {showPassword ? <EyeOff size={18} color="#333" /> : <Eye size={18} color="#555" />}
              </button>
            </div>
            {passwordError && (
              <p className="mt-2 text-sm" style={{ color: "#B85C38" }}>
                {passwordError}
              </p>
            )}
          </div>
